...
public class FaceRecognitionAppActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    private ArrayList<String> passLabels;
    ...
    private boolean checkinfor=false;
    ...
    private NativeMethods.TrainFacesTask.Callback trainFacesTaskCallback = new NativeMethods.TrainFacesTask.Callback() {
        ...
        public void onTrainFacesComplete(boolean result) {
            if (result){
                showToast("حاول مجددا تحديد الوجه", Toast.LENGTH_SHORT);
                uselocalimage=false;
            if(counter>3){
              ///  showToast("اعادة تعيئة", Toast.LENGTH_SHORT);

                refreashproj();
            }
            endwait();}
            else
                showToast(" فشل العملية", Toast.LENGTH_LONG);
        }
    };

    private void refreashproj() {
        AlertDialog.Builder builder = new AlertDialog.Builder(FaceRecognitionAppActivity.this);
        //builder.setTitle("Please enter your name:");
        View view =getLayoutInflater().inflate(R.layout.refreash_dialog,null);
        ...
        counter=0;
//                checkinfor=false;
//                nameLabels = tinydb.getListString("nameLabels");
//                passLabels = tinydb.getListString("passLabels");
            }
        });
    ...
    private void showLabelsDialog() {
        ...
        uniqueLabelsSet.remove("م");// Get all unique labels
        if (!uniqueLabelsSet.isEmpty()) { // Make sure that there are any labels
            ...
            final AlertDialog.Builder builder = new AlertDialog.Builder(FaceRecognitionAppActivity.this);


            builder.setTitle("الرجاء تاكيد بيانات الحساب");

            final EditText input1 = new EditText(FaceRecognitionAppActivity.this);
            ...
            final String[] uniqueLabels = uniqueLabelsSet.toArray(new String[uniqueLabelsSet.size()]); // Convert to String array for ArrayAdapter
            for (int i=0;i<uniqueLabels.length;i++)
            {
                uniqueLabels[i]=uniqueLabels[i].toLowerCase();
            }
            Arrays.sort(uniqueLabels);
            // Sort labels alphabetically
            builder.setPositiveButton("تحقق", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    dialog.dismiss();

                    String string1 = input1.getText().toString().trim();
                    Log.v("nameeeeeeeeee",string1);
                    ...
                    if (!string1.isEmpty()) {
                        ...
                        for (int i=0;i<uniqueLabels.length;i++)
                        {
                            Log.v("nameeeeeeeeee","label "+uniqueLabels[i]);
                            if(uniqueLabels[i].equals(string1)){
                                checkpass(string1);
                                correct=true;
                                break;
                            }
                        ...
                        if (!correct){
                            showToast("الاسم غير موجود", Toast.LENGTH_SHORT);
                        }
                    }

                }

                private void checkpass(String string1) {
                    Log.v("nameeeeeeeeee","checkpass");
                    AlertDialog.Builder builder2 = new AlertDialog.Builder(FaceRecognitionAppActivity.this);
                    builder2.setTitle("الرجاء ادخال كلمة المرور:");

                    final EditText input = new EditText(FaceRecognitionAppActivity.this);
                    input.setInputType(InputType.TYPE_CLASS_TEXT);
                    builder2.setView(input);

                    builder2.setPositiveButton("تحقق", null); // Set up positive button, but do not provide a listener, so we can check the string before dismissing the dialog
                    builder2.setNegativeButton("الغاء", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            dialog.dismiss();
                        }
                    });
                    builder2.setCancelable(false); // User has to input a name
                    final AlertDialog dialog1 = builder2.create();

                    // Source: http://stackoverflow.com/a/7636468/2175837
                    final String finalString = string1;
                            ...
                            mButton.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View view) {
                                    ...
                                    if (!string.isEmpty()) { // Make sure the input is valid
                                        int idname=nameLabels.indexOf(finalString);
                                        String pass=passLabels.get(idname);
                                        pass=pass.toLowerCase();
                                        if(pass.equals(string)){
                                            addLabel(finalString);
                                            checkinfor=true;
                                            namelocal=finalString;
                                            dialog.dismiss();
                                        }
                                        else
                                        {
                                            showToast("كلمة المرور خاطئة , الرجاء اعادة الادخال", Toast.LENGTH_SHORT);
    ...
    private void showEnterLabelDialog() {
        ...
        View view =getLayoutInflater().inflate(R.layout.dialog_enter,null);
        final EditText ename = (EditText)view.findViewById(R.id.ename);
        final EditText epass = (EditText)view.findViewById(R.id.epass);
        builder.setView(view);

        builder.setPositiveButton("ادخال", null); // Set up positive button, but do not provide a listener, so we can check the string before dismissing the dialog
        builder.setNegativeButton("الغاء", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
        ...
        dialog.setOnShowListener(new DialogInterface.OnShowListener() {
            ...
            public void onShow(final DialogInterface dialog) {
                ...
                mButton.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        String string = ename.getText().toString().trim();
                        String string1 = epass.getText().toString().trim();
                        string=string.toLowerCase();
                        string1=string1.toLowerCase();
                        nameLabels.add(string);
                        passLabels.add(string1);
                        tinydb.putListString("nameLabels", nameLabels);
                        tinydb.putListString("passLabels", passLabels);
                        takeimage(string);
                        showlist(nameLabels);
                        showlist(passLabels);
                        if (!string.isEmpty() && !string1.isEmpty()) { // Make sure the input is valid
    ...
    private void takeimage(String name) {
          String filename=Environment.getExternalStorageDirectory().getPath()+"/"+name+".jpeg";
        mOpenCvCameraView.takepic(filename);
        Log.d("tackpic", "file name " +filename);
         imagesurl.add(filename);
         tinydb.putListString("pathimage",imagesurl);
    }
    ...
    private void addtrainimage() {
        ...
        try {
            img = Utils.loadResource(this, R.drawable.mary);
            Imgproc.cvtColor(img, img, Imgproc.COLOR_RGB2BGRA);
            Imgproc.cvtColor(img,img,Imgproc.COLOR_RGB2GRAY);
            Log.v("imgnew","secuscee img");
        } catch (IOException e) {
            Log.e(TAG,Log.getStackTraceString(e));
        }
        Size imageSize = new Size(200, 200);
        Imgproc.resize(img, img, imageSize);
        Log.v("imgnew", "Small gray height: " + img.height() + " Width: " + img.width() + " total: " + img.total());


        Mat image = img.reshape(0, (int) img.total()); // Create column vector
        Log.i("imgnew", "Vector height: " + image.height() + " Width: " + image.width() + " total: " + image.total());
        ...
        if (images.size() > maximumImages) {
            ...
            Log.i("imgnew", "The number of images is limited to: " + images.size());
        }
...
//        });
        ...
        nameLabels = tinydb.getListString("nameLabels");
        passLabels = tinydb.getListString("passLabels");
        imagesurl=tinydb.getListString("pathimage");
        showlist(nameLabels);
        showlist(passLabels);
...
//        faceThreshold = mThresholdFace.getProgress(); // Get initial value
        faceThreshold = (float) 0.1; // Get initial value
...
//        });
     //   distanceThreshold = mThresholdDistance.getProgress(); // Get initial value
        distanceThreshold = (float) 0.3; // Get initial value
...
//        });
     //   maximumImages = (int)mMaximumImages.getProgress(); // Get initial value
        maximumImages = (int)10; // Get initial value
        useEigenfaces=true;
...
//            }
//        });
       Button find=(Button)findViewById(R.id.take_picture_button) ;
        ...
        findViewById(R.id.take_picture_button).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mMeasureDistTask != null && mMeasureDistTask.getStatus() != AsyncTask.Status.FINISHED) {
                    ...
                    showToast("سوف ياخذ بعض الوقت الرجاء الانتظار", Toast.LENGTH_SHORT);
                    return;
                }
                if (mTrainFacesTask != null && mTrainFacesTask.getStatus() != AsyncTask.Status.FINISHED) {
                    Log.i(TAG, "mTrainFacesTask is still running");
                    showToast("الرجاء الانتظار ", Toast.LENGTH_SHORT);
                    return;
                }
                counter++;
    ...
    private void showlist(ArrayList<String> nameLabels) {
        for(int i=0;i<nameLabels.size();i++)
        {
            Log.i(TAG,"i="+i+" ,"+nameLabels.get(i));
    ...
    private NativeMethods.MeasureDistTask.Callback measureDistTaskCallback = new NativeMethods.MeasureDistTask.Callback() {
        ...
        public void onMeasureDistComplete(Bundle bundle) {
            if (bundle == null) {
                showToast("خطأ في التعرف  ... الرجاء الانتظار", Toast.LENGTH_LONG);
                return;
            }

            float minDist = bundle.getFloat(NativeMethods.MeasureDistTask.MIN_DIST_FLOAT);
            if(uselocalimage==false) {
                if (minDist != -1) {
                    ...
                    if (imagesLabels.size() > minIndex) { // Just to be sure
                        ...
                        if (faceDist < faceThreshold && minDist < distanceThreshold) { // 1. Near face space and near a face class
                            ...
                            String n = imagesLabels.get(minIndex);
                            matimage=images.get(minIndex);
                            if( !n.equalsIgnoreCase("م"))
                            nextstep(n);
                            return;
                        } else if (faceDist < faceThreshold) // 2. Near face space but not near a known face class
                            showToast("وجة غير معروف.", Toast.LENGTH_LONG);
                        else if (minDist < distanceThreshold) // 3. Distant from face space and near a face class
                            showToast("خطأ في التعرف على الوجه.الرجاء اعادة المحاولة ", Toast.LENGTH_LONG);
                        else // 4. Distant from face space and not near a known face class.
                            showToast("الصورة لاتحوى وجه.", Toast.LENGTH_LONG);
                    }
                } else {
                    Log.w(TAG, "Array is null");
                    if (useEigenfaces || uniqueLabels == null || uniqueLabels.length > 1)
                        showToast("جاري التعرف", Toast.LENGTH_SHORT);
                    else
                        showToast("الرجاء اعاجة المحاولة", Toast.LENGTH_SHORT);
                }
                if(checkinfor==false)
                showLabelsDialog();
                else {
                    startwait();
                    namelocal=namelocal.toLowerCase();
                    addLabel(namelocal);
                    Log.w("localnamemmmmm", "localnale"+namelocal);

                }
            }
           if(uselocalimage==true)
               uselocalimage=false;
        }

        private void nextstep(String n) {
            if( n.equalsIgnoreCase("م")) {
               // showToast("فشل في التعرف على الوجه. ", Toast.LENGTH_LONG);
                Log.w("localnamemmmmm", "خطأ في التعرف على الوجه");

        }else {
                Intent intent = new Intent(FaceRecognitionAppActivity.this, NoteActivity.class);
                intent.putExtra(FaceRecognitionAppActivity.param, n);
        ...
        switch (requestCode) {
            case PERMISSIONS_REQUEST_CODE:
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    showToast("لاتوجد سماحية للمتابعة!", Toast.LENGTH_LONG);
                    finish();
                }
        }
    }
...
//    }
    ...
    private void flipCameraAnimation() {
        ...
        // Do flip camera animation
        View v2 = mToolbar.findViewById(R.id.delete);
...
//                for(int i=0;i<nameLabels.size();i++){
//                    ArrayList<String> nane=new ArrayList<>();
//                   nane= tinydb.getListString(nameLabels.get(i));
//                   nane.clear();
//                   tinydb.putListString(nameLabels.get(i),nane);
//                }
                images.clear(); // Clear both arrays, when new instance is created
                imagesLabels.clear();
                nameLabels.clear();
                ...
                imagesurl.clear();
               checkinfor=false;
                showToast("تم حذف الحسابات", Toast.LENGTH_SHORT);
                return true;
